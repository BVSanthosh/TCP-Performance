...
#CSV output file
measurements_file="quadratic_increase_delay_results.csv"
...
while true; do
    echo "Enter the value for the exponent"
    read exp

    if [[ -z "$exp" ]]; then
        echo "exponent not entered"
    else
        break
    fi
done
...
while true; do
    ...
    ((i++))
    delay=$(echo "$i^$exp" | bc)   #increases the delay
