...
#CSV output file
measurements_file="linear_increase_delay_results.csv"
...
while true; do
    echo "Enter delay increment to add after each TCP flow"
    read delay_inc

    if [[ -z "$delay_inc" ]]; then
        echo "Delay increment not entered"
    else
        break
    fi
done
...
while true; do
    ...
    delay=$((delay+delay_inc))   #increases the delay
